...
public class StatisticsAct extends BaseActivity{
	...
	private int searchtype = 4; // 查询时间类型：1 今天2 昨天 3 一周 4 本月5本年
	/**2016年4月第四周*/
...
//					radio2.setTextColor(Color.WHITE);
	...
	private String getCuurentData(int checkedId){
		...
		case R.id.radio0:
			return cal.get(Calendar.YEAR) + "年"
					+ cal.get(Calendar.MONTH) + "月第"
					+ cal.get(Calendar.WEEK_OF_MONTH)+"周";
		case R.id.radio1:
			return cal.get(Calendar.YEAR) + "年"
	...
	protected void initData() {
		...
		setColor("0");
		MainPageDataV2Bean bean = (MainPageDataV2Bean) getIntent().getSerializableExtra("todayBean");
...
//		searchtype = current;
	...
	private void setWeekData(String data) {
		...
		if(searchtype ==3){//本周
			weekBean = JsonUtil.parseJsonToBean(data,
			...
			YearBean = JsonUtil.parseJsonToBean(data,
					MoreDataOfWeekBean.class);
			moreDataBean = YearBean;
		}
...
//			tvTotal.setText("共"+totalWeek+"人");
	...
	private void setColor(String text){
        ...
        spanBuilder.setSpan(new TextAppearanceSpan(null, 0, 30, redColors, null), 1, text.length()+1, Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
