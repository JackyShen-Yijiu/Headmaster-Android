...
public class Bar {
	...
	protected float[] calcBarHeightAndMargin(float YSteps, int barNumber) {
		...
		float labelBarTotalHeight = MathHelper.getInstance().mul(YSteps,
				mBarTickSpacePercent);
		float barTotalInnerMargin = MathHelper.getInstance().mul(
				labelBarTotalHeight, (float) mBarInnerMargin);
		float barInnerMargin = MathHelper.getInstance().div(
				barTotalInnerMargin, barNumber);
	...
	protected float[] calcBarWidthAndMargin(float XSteps, int barNumber) {
		...
		float labelBarTotalWidth = MathHelper.getInstance().mul(XSteps,
				mBarTickSpacePercent);
		float barTotalInnerMargin = MathHelper.getInstance().mul(
				labelBarTotalWidth, (float) mBarInnerMargin);

		float barTotalWidth = MathHelper.getInstance().sub(labelBarTotalWidth,
				barTotalInnerMargin);
		float barInnerMargin = MathHelper.getInstance().div(
				barTotalInnerMargin, barNumber);
	...
	protected void drawBarItemLabel(String text, float x, float y, Canvas canvas) {
		...
		if (getItemLabelsVisible() && text.length() > 0) {
			...
			case VERTICAL:
				...
				break;
			case HORIZONTAL:
				float textWidth = DrawHelper.getInstance().getTextWidth(
						getItemLabelPaint(), text);
				switch (mItemLabelStyle) {
				case OUTER:
			...
			DrawHelper.getInstance().drawRotateText(text, cx, cy,
					getItemLabelRotateAngle(), canvas, getItemLabelPaint());
		}
	}
