...
public class AxesChart extends EventChart {
	...
	private float mxMargin = -10.f;// 25.0f; //散点和气泡要注意下
		...
		for (int i = 0; i < lstLabels.size(); i++) {
			...
			case VERTICAL_CENTER:
				if (!t.isShowTickMarks() || !isDrawYAxisTickMarks(t.Y, mMoveY))
					showTicks = false;
				categoryAxis.renderAxisHorizontalTick(this.getLeft(), this
						.getPlotArea().getLeft(), canvas, t.X, t.Y, t.Label,
						t.labelX, t.labelY, showTicks);
				break;
			case TOP: // X
			case BOTTOM:
			case HORIZONTAL_CENTER:
				...
				categoryAxis.renderAxisVerticalTick(canvas, t.X, t.Y, t.Label,
						t.labelX, t.labelY, showTicks, oe);
				break;
	...
	protected void drawDataAxisLabels(Canvas canvas,
		...
		for (int i = 0; i < lstLabels.size(); i++) {
			...
			case RIGHT:
			case VERTICAL_CENTER:
				dataAxis.renderAxisHorizontalTick(this.getLeft(), this
						.getPlotArea().getLeft(), canvas, t.X, t.Y, t.Label,
	...
	protected boolean isDrawXAxisTickMarks(float currentX, float moveX) {
		if (Float.compare(currentX + moveX, plotArea.getLeft()) == -1)
			return false;
		if (Float.compare(currentX + moveX, plotArea.getRight()) == 1)
			return false;
		return true;
	}

	// 横向网格线
	protected void drawHorizontalGridLines(Canvas canvas, float plotLeft,
			float plotRight, int tickID, int tickCount, float YSteps,
			float currentY) {
		if (tickID < 0)
			return; // tickID <= 0
		...
		if (tickID > 0) {
			if (tickID % 2 != 0) {
				plotGrid.renderOddRowsFill(canvas, plotLeft,
						add(currentY, YSteps), plotRight, currentY);
			} else {
	...
	// 绘制竖向网格线
	protected void drawVerticalGridLines(Canvas canvas, float plotTop,
			float plotBottom, int tickID, int tickCount, float XSteps,
			float currentX) {
		// 绘制竖向网格线
		if (plotGrid.isShowVerticalLines()) {
			// if (i > 0 && i + 1 < tickCount) //line
			plotGrid.renderGridLinesVertical(canvas, currentX, plotBottom,
					currentX, plotTop);
		}
	}
	...
	protected void drawClipAxisClosed(Canvas canvas) {
		...
		case RIGHT:
		case VERTICAL_CENTER:
			dataAxis.renderAxisLine(canvas, plotLeft, plotTop, plotLeft,
					plotBottom);
			dataAxis.renderAxisLine(canvas, plotRight, plotTop, plotRight,
					plotBottom);
			break;
		case TOP:
		case BOTTOM:
		case HORIZONTAL_CENTER:
			dataAxis.renderAxisLine(canvas, plotLeft, plotTop, plotRight,
					plotTop);
			dataAxis.renderAxisLine(canvas, plotLeft, plotBottom, plotRight,
					plotBottom);
			break;
		}
		...
		case RIGHT:
		case VERTICAL_CENTER:
			categoryAxis.renderAxisLine(canvas, plotLeft, plotBottom, plotLeft,
					plotTop);
			categoryAxis.renderAxisLine(canvas, plotRight, plotBottom,
					plotRight, plotTop);
			break;
		case TOP:
		case BOTTOM:
		case HORIZONTAL_CENTER:
			categoryAxis.renderAxisLine(canvas, plotLeft, plotTop, plotRight,
					plotTop);
			categoryAxis.renderAxisLine(canvas, plotLeft, plotBottom,
	...
	protected void drawClipDataAxisLine(Canvas canvas) {
		...
		case LEFT:
			...
			break;
		case RIGHT:
			dataAxis.renderAxis(canvas, plotRight, plotTop, plotRight,
					plotBottom);
			break;
		case VERTICAL_CENTER:
		...
		case TOP:
			...
			break;
		case BOTTOM:
			dataAxis.renderAxis(canvas, plotLeft, plotBottom, plotRight,
					plotBottom);
			break;
		case HORIZONTAL_CENTER:
	...
	protected void drawClipCategoryAxisLine(Canvas canvas) {
		...
		switch (mCategoryAxisLocation) {
		case LEFT:
			categoryAxis.renderAxis(canvas, plotLeft, plotBottom, plotLeft,
					plotTop);
			break;
		case RIGHT:
			categoryAxis.renderAxis(canvas, plotRight, plotTop, plotRight,
					plotBottom);
			break;
		case VERTICAL_CENTER:
			...
			break;
		case TOP:
			categoryAxis.renderAxis(canvas, plotLeft, plotTop, plotRight,
					plotTop);
			break;
		case BOTTOM:
			categoryAxis.renderAxis(canvas, plotLeft, plotBottom, plotRight,
					plotBottom);
			break;
		case HORIZONTAL_CENTER:
	...
	protected boolean drawClipVerticalPlot(Canvas canvas) {
		...
		// 设置图显示范围
		canvas.save();
		canvas.clipRect(this.getLeft(), this.getTop(), this.getRight(),
				this.getBottom());

		if (XEnum.PanMode.VERTICAL == this.getPlotPanMode()
				|| XEnum.PanMode.FREE == this.getPlotPanMode()) {
			if (getPlotGrid().isShowVerticalLines())
				gWidth = this.getPlotGrid().getVerticalLinePaint()
						.getStrokeWidth();
			// 绘制Y轴tick和marks
			canvas.save();
			canvas.clipRect(plotArea.getLeft() - gWidth, plotArea.getTop()
					- gWidth, plotArea.getRight() + gWidth,
					plotArea.getBottom() + gWidth);
		...
		if (XEnum.PanMode.HORIZONTAL == this.getPlotPanMode()
			...
			if (getPlotGrid().isShowHorizontalLines())
				gWidth = this.getPlotGrid().getHorizontalLinePaint()
						.getStrokeWidth();
			// 绘制X轴tick和marks
			canvas.save();
		...
		if (XEnum.PanMode.HORIZONTAL == this.getPlotPanMode()
			...
			canvas.clipRect(this.getLeft() + xMargin, this.getTop(),
					this.getRight() - xMargin, this.getBottom()); // this.getRight()
																	// + xMargin
