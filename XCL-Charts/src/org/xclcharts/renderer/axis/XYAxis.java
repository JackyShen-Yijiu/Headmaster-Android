...
public class XYAxis extends Axis {
	...
	protected String getFormatterLabel(String text) {
		...
		if (itemLabel.endsWith(".0")) {
			itemLabel = itemLabel.substring(0, itemLabel.length() - 2);
		}
		if ("0".equals(itemLabel)) {
			itemLabel = "";
		}
		return itemLabel;
	}
	...
	protected void renderHorizontalTick(float chatLeft, float plotLeft,
			...
			float labelX, float labelY, boolean isTickVisible) {
		...
		case LEFT: {
			if (isShowTickMarks()) {
				marksStartX = MathHelper.getInstance().sub(centerX,
						getTickMarksLength());
				markeStopX = centerX;
			}

			if (this.isShowAxisLabels())
				labelStartX = MathHelper.getInstance().sub(marksStartX,
						getTickLabelMargin());
			break;
		}
		case CENTER: {
			if (isShowTickMarks()) {
				marksStartX = MathHelper.getInstance().sub(centerX,
						getTickMarksLength() / 2);
				markeStopX = MathHelper.getInstance().add(centerX,
						getTickMarksLength() / 2);
			}
			break;
		}
		case RIGHT:
			...
			if (this.isShowAxisLabels())
				labelStartX = MathHelper.getInstance().add(markeStopX,
						getTickLabelMargin());
		...
		if (isShowAxisLabels()) {
			renderHorizontalTickLabels(chatLeft, plotLeft, canvas, labelStartX,
					labelStartY, markeStopX, text);
		}
	...
	private void renderHorizontalTickLabels(float chatLeft, float plotLeft,
		...
		if (Align.LEFT == getHorizontalTickAlign()) // 处理多行标签
		{
			...
			renderLeftAxisTickMaskLabel(canvas, labelStartX, labelStartY
					+ textHeight, text, width);
		} else {
			DrawHelper.getInstance().drawRotateText(getFormatterLabel(text),
					labelStartX, labelStartY + textHeight,
	 ...
	 *            标签文本
	 */
	protected void renderVerticalTick(Canvas canvas, float centerX,
		...
		case TOP: {
			...
			if (isShowTickMarks()) {
				marksStartY = MathHelper.getInstance().sub(centerY,
						getTickMarksLength());
				marksStopY = centerY;
			}
			marksStartY = MathHelper.getInstance().sub(centerY,
		...
		case MIDDLE: {
			if (isShowTickMarks()) {
				marksStartY = MathHelper.getInstance().sub(centerY,
						getTickMarksLength() / 2);
				marksStopY = MathHelper.getInstance().add(centerY,
						getTickMarksLength() / 2);
			}
			break;
		}
		...
		if (isShowTickMarks() && isTickVisible) {
			...
			// {
			// }else{
			canvas.drawLine(centerX, mstartX,
			// marksStartY - this.getAxisPaint().getStrokeWidth() / 2,
					centerX, marksStopY, getTickMarksPaint());
			// }
		}
	...
	private void renderLeftAxisTickMaskLabel(Canvas canvas, float centerX,
			float centerY, String text, float width) {
		if (!isShowAxisLabels())
			return;
		...
		double txtLength = DrawHelper.getInstance().getTextWidth(
				getTickLabelPaint(), label);
		if (txtLength <= width) {
			// 标签绘制在一行中
			...
			for (int i = 0; i < label.length(); i++) {
				...
				totalWidth = MathHelper.getInstance()
						.add(totalWidth, charWidth);
				if (Float.compare(totalWidth, width) == 1) {
					DrawHelper.getInstance().drawRotateText(lnString, centerX,
							renderY, getTickLabelRotateAngle(), canvas,
							getTickLabelPaint());
	...
	// 绘制轴线
	protected void drawAxisLine(Canvas canvas, float startX, float startY,
		...
		if (XEnum.AxisLineStyle.CAP == mAxisLineStyle
			...
			if (Float.compare(startY, stopY) != 0) // 竖轴
			{
				...
				if (XEnum.AxisLineStyle.FILLCAP == mAxisLineStyle) {
					...
					canvas.drawLine(startX, startY, stopX, axisEnd,
							this.getAxisPaint());
				} else {
					canvas.drawLine(startX, startY, stopX, angleTop,
							this.getAxisPaint());
					canvas.drawLine(angleLeftX, axisEnd, stopX, angleTop,
							this.getAxisPaint());
					canvas.drawLine(angleRightX, axisEnd, stopX, angleTop,
							this.getAxisPaint());
				}
			} else {
				...
				if (XEnum.AxisLineStyle.FILLCAP == mAxisLineStyle) {
					...
					path.close();
					canvas.drawPath(path, getAxisPaint());
					canvas.drawLine(startX, startY, axisEnd, stopY,
							this.getAxisPaint());
				} else {
					canvas.drawLine(startX, startY, angleTop, stopY,
							this.getAxisPaint());
					canvas.drawLine(axisEnd, angleTopY, angleTop, stopY,
							this.getAxisPaint());
					canvas.drawLine(axisEnd, angleBottom, angleTop, stopY,
							this.getAxisPaint());
				}
			}
		} else { // none
