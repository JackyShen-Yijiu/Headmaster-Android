...
public class PieChart extends CirChart {
	...
	private RadialGradient renderRadialGradient(Paint paintArc, float cirX, float cirY, float radius) {
		...
		RadialGradient radialGradient = new RadialGradient(cirX, cirY, radialRadius, darkerColor, color,
		...
		try {
			...
			for (int i = 0; i < count; i++) {
				...
				if (cData.getSelected()) { // 指定突出哪个块
					...
					// 计算百分比标签
					PointF point = MathHelper.getInstance().calcArcEndPointXY(cirX, cirY, newRadius,
				...
				// 保存角度
				saveArcRecord(i, cirX + this.mTranslateXY[0], cirY + this.mTranslateXY[1], radius, offsetAngle,
						currentAngle, mSelectedOffset, getOffsetAngle());
...
//			plotLegend.renderPieKey(canvas, this.mDataset);
	...
	protected boolean validateParams() {
		...
		for (PieData cData : mDataset) {
			...
			// Log.e(TAG,"圆心角:"+Float.toString(currentValue)+"
			// 合计:"+Float.toString(totalAngle));
			if (Float.compare(totalAngle, 0.0f) == -1) {
				Log.w(TAG, "传入参数不合理，圆心角总计小于等于0度. 现有圆心角合计:" + Float.toString(totalAngle) + " 当前圆心角:"
						+ Float.toString(currentValue) + " 当前百分比:" + Double.toString(cData.getPercentage()));
				// return false;
			} else if (Float.compare(totalAngle, getTotalAngle() + 0.5f) == 1) {
				// 圆心角总计大于360度
				Log.w(TAG, "传入参数不合理，圆心角总计大于总角度. 现有圆心角合计:" + Float.toString(totalAngle));
				// return false;
			}
		}
		return true;
